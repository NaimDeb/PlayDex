...
class FollowedGamesDeleteProcessorTest extends TestCase
{
    private EntityManagerInterface $entityManager;
    private Security $security;
    private EntityRepository $gameRepository;
    private EntityRepository $followedGamesRepository;
    private FollowedGamesDeleteProcessor $processor;
    ...
    protected function setUp(): void
    {
        ...
        $this->processor = new FollowedGamesDeleteProcessor($this->entityManager, $this->security);
    }

    public function testProcessDeletesFollowedGame(): void
    {
        $game = new Game();
        $existing = new FollowedGames();

        $this->gameRepository->method('find')->with(1)->willReturn($game);
        $this->followedGamesRepository->method('findOneBy')->willReturn($existing);

        $this->entityManager->expects($this->once())->method('remove')->with($existing);
