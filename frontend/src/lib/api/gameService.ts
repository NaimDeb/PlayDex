...
class GameService {
  ...
  async patchPatchnote(
    id: string,
    patchnoteData: Partial<Patchnote>
  ): Promise<Patchnote> {
    const config = {
  ...
  async getModificationsByPatchnoteId(
    id: string,
    page: number
  ): Promise<Array<Modification>> {
    const config = authUtils.getAuthorization();
    const response = await apiClient.get(
      `/modifications?page=${page}&patchnote.id=${id}`,
      config
    );
